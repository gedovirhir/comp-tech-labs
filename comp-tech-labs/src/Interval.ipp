...
template<typename T>
Interval<T>::Interval(const Interval& other) : bot(other.bot), top(other.top) {};
...
template<typename T>
Interval<T>::Interval(T&& b, T&& t) : bot(forward<T>(b)), top(forward<T>(t)) {};
...
Interval<T> Interval<T>::cross_operate(const function<T(T, T)>& a_func, const Interval& other) const {
	return Interval(
		utils::min(bot * other.bot, bot * other.top, top * other.bot, top * other.top),
		utils::max(bot * other.bot, bot * other.top, top * other.bot, top * other.top)
	);
}
...
template<typename T>
Interval<T> Interval<T>::operator-(const Interval& other) const {
	return Interval(bot - other.top, top - other.bot);
};
...
Interval<T> Interval<T>::operator^(const T& other) const {
	T new_b = pow(bot, other);
	T new_t = pow(top, other);
